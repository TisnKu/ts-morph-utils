...
import { Identifier, ImportDeclaration, Node, SyntaxKind } from "ts-morph";
...
export default function (tsConfigFilePath?: string, projectPath?: string) {
  ...
  const sourceFile = project.getSourceFile((sourceFile) => {
    const firstVariableDeclaration = sourceFile.getFirstDescendantByKind(
      SyntaxKind.VariableDeclaration
    );
    return !!firstVariableDeclaration?.hasExportKeyword();
  });

  const otherSourceFile = project
    .getSourceFiles()
  ...
  _.chain(otherSourceFile)
    ...
    .flatten()
    .filter(
      (identifier) => identifier.getText() === variableDeclaration.getName()
    )
    .value()
    .forEach((i) => {
      ...
      if (definitionNode === variableDeclaration) {
        ...
        const belongingImportSpecifier: Node = (
          i as Identifier
        ).getFirstAncestorByKind(SyntaxKind.ImportSpecifier);
        if (Node.isImportSpecifier(belongingImportSpecifier)) {
          const importDeclaration: ImportDeclaration =
            belongingImportSpecifier.getFirstAncestorByKind(
          ...
          queue.push(() =>
            i.replaceWithText(variableDeclaration.getInitializer().getText())
          );
        }
      }
    });
